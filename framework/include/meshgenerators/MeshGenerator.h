...
protected:
   ...
   */
  [[nodiscard]] std::unique_ptr<CSG::CSGBase> & getCSGBase(const std::string & param_name,
                                                           const MeshGeneratorName mg_name);

  /**
   * Like \p getCSGBase(), but takes the name of another MeshGenerator directly.
   *
   * NOTE: You MUST catch this by reference!
   *
   * @param mesh_generator_name The name of the MeshGenerator associated with the CSGBase object
   * @return The CSGBase pointer generated by that MeshGenerator
   */
  [[nodiscard]] std::unique_ptr<CSG::CSGBase> &
  getCSGBaseByName(const MeshGeneratorName & mesh_generator_name, const MeshGeneratorName mg_name);

  /**
   * Like \p getCSGBase(), but for multiple generators.
   *
   * NOTE: You MUST catch this by reference!
   *
   * @param param_name The name of the parameter that contains the names of MeshGenerator objects
   * @return Vector of CSGBase pointers associated with input parameter
   */
  [[nodiscard]] std::vector<std::unique_ptr<CSG::CSGBase> *>
  getCSGBases(const std::string & param_name, const MeshGeneratorName mg_name);

  /**
   * Like \p getCSGBaseByName(), but for multiple generators.
   *
   * NOTE: You MUST catch this by reference!
   *
   * @param mesh_generator_names The names of MeshGenerator objects
   * @return Vector of CSGBase pointers associated with input parameter
   */
  [[nodiscard]] std::vector<std::unique_ptr<CSG::CSGBase> *>
  getCSGBasesByName(const std::vector<MeshGeneratorName> & mesh_generator_names,
                    const MeshGeneratorName mg_name);
