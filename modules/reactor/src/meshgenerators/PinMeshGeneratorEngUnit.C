...
#include "PinMeshGeneratorEngUnit.h"
...
registerMooseObject("ReactorApp", PinMeshGeneratorEngUnit);

InputParameters
PinMeshGeneratorEngUnit::validParams()
{
  ...
  params.addParam<bool>("expand_unit", false, "Whether to expand the engineering unit into CSG components");
...
PinMeshGeneratorEngUnit::PinMeshGeneratorEngUnit(const InputParameters & parameters)
...
void
PinMeshGeneratorEngUnit::generateFlexibleAssemblyBoundaries()
{
...
void
PinMeshGeneratorEngUnit::generateMetadata()
{
...
PinMeshGeneratorEngUnit::generate()
{
...
PinMeshGeneratorEngUnit::generateCSG()
{
  ...
  std::string prev_eng_unit_name = "";
  // Define pin engineering unit for each radial ring in pin definition
  if (_ring_radii.size())
  {
    std::vector<std::string> fill_mats;
    for (const auto i : index_range(_ring_radii))
      fill_mats.push_back("rgmb_region_" + std::to_string(_region_ids[0][i]));
    const auto pin_unit_name = name() + "_pin";
    std::unique_ptr<CSG::PinEngUnit> pin_ptr =
        std::make_unique<CSG::PinEngUnit>(pin_unit_name, _ring_radii, fill_mats);
    csg_obj->addEngUnit(std::move(pin_ptr));
    prev_eng_unit_name = pin_unit_name;
  }

  unsigned int duct_index = 0;
  // Add polygon domain units corresponding to pin ducts and outer boundary
  for (const auto & duct_halfpitch : _duct_halfpitch)
  {
    std::string eng_unit_name = name() + "_duct_" + std::to_string(duct_index);
    
    std::string fill_mat = "rgmb_region_" + std::to_string(_region_ids[0][_ring_radii.size() + duct_index]);
    std::unique_ptr<CSG::PolygonDomainEngUnit> poly_ptr =
        std::make_unique<CSG::PolygonDomainEngUnit>(eng_unit_name, 6, duct_halfpitch, fill_mat, prev_eng_unit_name);
    csg_obj->addEngUnit(std::move(poly_ptr));
    prev_eng_unit_name = eng_unit_name;
    ++duct_index;
  }

  std::string fill_mat = "rgmb_region_" + std::to_string(_region_ids[0][_ring_radii.size() + duct_index]);
  std::string eng_unit_name = name() + "_outer_boundary";
  std::unique_ptr<CSG::PolygonDomainEngUnit> poly_ptr =
      std::make_unique<CSG::PolygonDomainEngUnit>(eng_unit_name, 6, _pitch / 2., fill_mat, prev_eng_unit_name);
  csg_obj->addEngUnit(std::move(poly_ptr));
